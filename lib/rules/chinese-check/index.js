...
function check (cont, node, text, isSimplified, offset) {
  ...
  if (typeof text === 'string') {
    ...
    if (replaceArr.length > 0) {
      ...
      cont.report({
        ...
        fix: fixer => {
          return fixer.replaceTextRange([node.range[0] + offset, node.range[1] - offset], newText)
        },
      })
    }
  }
  ...
  create: function (context) {
    ...
    return {
      TemplateElement: (node) => {
        check(context, node, node.value.raw, isSimplified, 1)
      },
      Literal: (node) => {
